...
export function blockerGuidanceForReason(reason) {
  ...
  if (!raw) return null;
  if (text.includes("docker control") || text.includes("docker_control") || text.includes("docker cli")) return { raw, happened: "This entry points to a retired deployment workflow.", next: "Open Models and use Get GGUF, or download/import a compatible GGUF in Discover Models, then load it with the native runtime." };
